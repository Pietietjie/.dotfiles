...
M.tailwind = {}
for name, shades in pairs(tw_raw) do
  for i = 1, #shades, 2 do
    M.tailwind[name .. '-' .. shades[i]] = shades[i + 1]
  end
end
...
function M.hsl_to_hex(h, s, l_pct)
  h = h % 360
  local s1 = M.clamp(s, 0, 100) / 100
  local l1 = M.clamp(l_pct, 0, 100) / 100
  local c = (1 - math.abs(2 * l1 - 1)) * s1
